...
def scalar_vector_multiplication(k, v, q):
    n = len(v)
    return [k * v[i] % q for i in range(n)]
...
def vector_oppose(v, q):
    n = len(v)
    return [- v[i] % q for i in range(n)]

def vector_vector_multiplication(m, v, q):
    n = len(m)
    return [m[i] * v[i] % q for i in range(n)]
